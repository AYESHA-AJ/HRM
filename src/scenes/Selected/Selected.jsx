...
const Selected = () => {
  ...
  useEffect(() => {
    // Filter applicants with both selected and shortlisted statuses being true
    const filteredApplicants = applicantData.filter(applicant => applicant.selected && applicant.shortlisted);
    ...
    <Box m="20px">
      ...
      <Box mt={3} style={{ height: 400, width: '100%' }}>
        <DataGrid
          ...
          { field: 'status Two', headerName: 'Status Selected', width: 150,
          renderCell: (params) => (
            <Typography style={{ color: params.row.selected ? blue[500] : grey[500] }}>
              {params.row.shortlisted ? 'Selected' : 'Not Selected'}
            </Typography>
          ),
        },
            {
